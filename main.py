...
async def submit_test(data: dict):
    ...
    # Формуємо список топ-результатів (1 або кілька, якщо нічия)
    top_results = []
    for t in top_types:
        type_info = types_data.get(t, {})
        top_results.append({
            "personality_type": int(t),
    ...
    # Залишаємо старі поля для сумісності (перший тип),
    # але додаємо нові: top_types, top_results
    primary = top_results[0] if top_results else {
        "personality_type": 0,
        "type_name": "",
        "subtitle": "",
        "short_description": "",
    ...
    result = {
        **primary,
        "top_types": [int(t) for t in top_types],
        "top_score": max_score,
